#clean data and marketing analysis
# Importing libraries
library(data.table)
library(readxl)               
library(tidyverse)
...
retail <- retail %>%
  ...
  #mutate(Date = as.factor(format(InvoiceDate,"%M/%D/%YY")))
...
# no1.What items do people buy more often?
retail %>% 
...
# No.2 Top 10 most sold products represent around 3% of total items sold by the company
retail %>% 
  group_by(Description) %>% 
  summarize(count = n()) %>% 
...
# No.3 How many items does each customer buy?
retail %>% 
  group_by(InvoiceNo) %>% 
...
# No.4 What is the average value per order?
retail %>% 
...
# No.5 Which countries do they sell their goods to?
treemap(retail,
        index      = c("Country"),
        vSize      = "Quantity",
        title      = "",
